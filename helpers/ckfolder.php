...
abstract class CKFolder
{
	...
	public static function copy($src, $dest, $path = '', $force = false, $use_streams = false)
	{
		...
		if ($path)
		{
			$src = CKPath::clean($path . '/' . $src);
			$dest = CKPath::clean($path . '/' . $dest);
		}
			...
			while (($file = readdir($dh)) !== false)
			{
				...
				switch (filetype($sfid))
				{
					...
					case 'file':
						...
						$dfid = CKPath::clean(str_replace(MOBILEMENUCK_PATH, $FTPOptions['root'], $dfid), '/');
	...
	public static function create($path = '', $mode = 0755)
	{
		...
		if (!self::exists($parent))
		{
			...
			{
				// JLog::add(__METHOD__ . ': ' . CKText::_('JLIB_FILESYSTEM_ERROR_FOLDER_LOOP'), JLog::WARNING, 'jerror');
				$nested--;
			...
			{
				// CKFolder::create throws an error
				$nested--;
		...
		/*if ($FTPOptions['enabled'] == 1)
		{
			...
			$path = CKPath::clean(str_replace(MOBILEMENUCK_PATH, $FTPOptions['root'], $path), '/');
			$ret = $ftp->mkdir($path);
			$ftp->chmod($path, $mode);
		}
		else
...
/*
			...
			if ($obd != null)
			{
				...
				if ($inBaseDir == false)
				{
					// Return false for CKFolder::create because the path to be created is not in open_basedir
					// JLog::add(__METHOD__ . ': ' . CKText::_('JLIB_FILESYSTEM_ERROR_FOLDER_PATH'), JLog::WARNING, 'jerror');
...
*/
			...
			if (!$ret = @mkdir($path, $mode))
			{
				@umask($origmask);
				// JLog::add(
					// __METHOD__ . ': ' . CKText::_('JLIB_FILESYSTEM_ERROR_COULD_NOT_CREATE_DIRECTORY') . 'Path: ' . $path, JLog::WARNING, 'jerror'
				// );
	...
	public static function delete($path)
	{
		...
		if (!$path)
		{
			...
			// JLog::add(__METHOD__ . ': ' . CKText::_('JLIB_FILESYSTEM_ERROR_DELETE_BASE_DIRECTORY'), JLog::WARNING, 'jerror');
		...
		if (!is_dir($path))
		{
			// JLog::add(CKText::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER', $path), JLog::WARNING, 'jerror');
		...
		if (!empty($files))
		{
			...
			if (CKFile::delete($files) !== true)
			{
				// CKFile::delete throws an error
				return false;
			}
		}
		...
		foreach ($folders as $folder)
		{
			...
			{
				// CKFolder::delete throws an error
				return false;
			}
		}
		...
		{
			// JLog::add(CKText::sprintf('JLIB_FILESYSTEM_ERROR_FOLDER_DELETE', $path), JLog::WARNING, 'jerror');
			$ret = false;
		}
	...
	public static function move($src, $dest, $path = '', $use_streams = false)
	{
		...
		if (!self::exists($src))
		{
			return CKText::_('Can not find source folder');
		}

		if (self::exists($dest))
		{
			return CKText::_('Folder already exists');
		}
		...
		if ($use_streams)
		{
			...
			{
				return CKText::sprintf('JLIB_FILESYSTEM_ERROR_FOLDER_RENAME', $stream->getError());
			}
		...
		else
		{
			/*if ($FTPOptions['enabled'] == 1)
			{
				...
				// Translate path for the FTP account
				$src = CKPath::clean(str_replace(MOBILEMENUCK_PATH, $FTPOptions['root'], $src), '/');
				$dest = CKPath::clean(str_replace(MOBILEMENUCK_PATH, $FTPOptions['root'], $dest), '/');

				// Use FTP rename to simulate move
				if (!$ftp->rename($src, $dest))
				{
					return CKText::_('Rename failed');
				}
	...
	{
		return is_dir(CKPath::clean($path));
	}
		...
		$excludefilter = array('^\..*', '.*~'), $naturalSort = false)
	{
		...
		if (!is_dir($path))
		{
			// JLog::add(CKText::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER_FILES', $path), JLog::WARNING, 'jerror');
		...
		$excludefilter = array('^\..*'))
	{
		...
		if (!is_dir($path))
		{
			// JLog::add(CKText::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER_FOLDER', $path), JLog::WARNING, 'jerror');
	...
	public static function listFolderTree($path, $filter, $maxLevel = 3, $level = 0, $parent = 0)
	{
		...
		if ($level == 0)
		{
			$GLOBALS['_CKFolder_folder_tree_index'] = 0;
		}
		...
		if ($level < $maxLevel)
		{
			...
			{
				$id = ++$GLOBALS['_CKFolder_folder_tree_index'];
				$fullName = CKPath::clean($path . '/' . $name);
				$dirs[] = array('id' => $id, 'parent' => $parent, 'name' => $name, 'fullname' => $fullName,
					'relname' => str_replace(MOBILEMENUCK_PATH, '', $fullName));
